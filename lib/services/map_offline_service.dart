...
class MapOfflineService {
  ...
  static const Map<String, String> _tileRequestHeaders = <String, String>{
    'User-Agent': 'EcoGuide/1.0 (contact: support@ecoguide.local)',
    'Accept': 'image/png,image/*;q=0.8,*/*;q=0.5',
  };

  final http.Client _httpClient;

  /// Shared base tile path across all instances (set once by initialize).
  static String? _sharedBaseTilePath;

  String? get _baseTilePath => _sharedBaseTilePath;
...
class LocalFirstTileProvider extends TileProvider {
  ...
  final NetworkTileProvider _networkTileProvider;
  static final Uint8List _transparentTileBytes = base64Decode(
    'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9WmW5QAAAABJRU5ErkJggg==',
  );
  ...
  LocalFirstTileProvider({MapOfflineService? service}) 
    : _mapOfflineService = service ?? MapOfflineService(),
      _networkTileProvider = NetworkTileProvider(
        headers: MapOfflineService._tileRequestHeaders,
      );
  ...
  ImageProvider getImage(TileCoordinates coordinates, TileLayer options) {
    ...
    if (!ConnectivityService.instance.isOnline) {
      // No internet available: keep map interactive with local fallback only.
      return MemoryImage(_transparentTileBytes);
    }

    return _networkTileProvider.getImage(coordinates, options);
  }
}

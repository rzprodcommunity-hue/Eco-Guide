...
class _OfflineTrailsScreenState extends State<OfflineTrailsScreen> {
  ...
  late final LocalServiceService _localServiceService;
  ...
  final double _deviceStorageGb = 128;
  ...
  void initState() {
    ...
    _offlineService = OfflineService(apiClient);
    _mapOfflineService = MapOfflineService();
    _trailService = TrailService(apiClient);
    _poiService = PoiService(apiClient);
    _localServiceService = LocalServiceService(apiClient);
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      if (!connectivity.isOfflineMode) {
        await connectivity.forceCheckBackend(ApiConstants.baseUrl);
      }

      if (connectivity.isOfflineMode) {
        _allTrails = await OfflineCacheService.instance.getOfflineTrails();
      } else {
        try {
          _allTrails = await _trailService.getAllTrails();
  ...
  Future<void> _installPackage(String packageId) async {
    setState(() {
      ...
      _downloadStatusText[packageId] = 'Preparation...';
    });
    ...
    if (connectivity.isOfflineMode) {
      _showMessage(
        'Mode hors ligne actif: passez en mode connecte pour telecharger ce package.',
      );
      if (mounted) {
        setState(() {
    ...
    try {
      if (packageId == 'topo') {
        setState(() {
          _downloadProgress[packageId] = 0.05;
          _downloadStatusText[packageId] = 'Telechargement des sentiers...';
        });

        final allTrails = await _trailService.getAllTrails(pageSize: 100);
        if (allTrails.isEmpty) {
          throw Exception('Aucun sentier recu depuis le backend.');
        }

        _allTrails = allTrails;
        _regionTrails = _resolveTabarkaRegion(_allTrails);

        await OfflineCacheService.instance.replaceTrails(
          allTrails,
          _resolution,
          packageSizeMb.toDouble(),
        );

        setState(() {
          _downloadProgress[packageId] = 0.2;
          _downloadStatusText[packageId] = 'Mise en cache des images sentiers...';
        });

        var processedTrails = 0;
        for (final trail in allTrails) {
          ...
          processedTrails++;
          if (mounted && allTrails.isNotEmpty) {
            final progress = 0.2 + ((processedTrails / allTrails.length) * 0.15);
            setState(() {
              _downloadProgress[packageId] = progress;
              _downloadStatusText[packageId] = 'Images sentiers: $processedTrails / $allTrails.length';
            });
          }
        }
        ...
        final mapResult = await _mapOfflineService.downloadTabarkaTiles(
          ...
          onProgress: (progress, downloaded, total) {
            if (mounted) {
              setState(() {
                _downloadProgress[packageId] = 0.35 + (progress * 0.65);
        ...
        setState(() {
          _downloadProgress[packageId] = 0.05;
          _downloadStatusText[packageId] = 'Telechargement de tous les POI...';
        });

        final allPois = await _poiService.getAllPois(pageSize: 100);
        if (allPois.isEmpty) {
          throw Exception('Aucun POI recu depuis le backend.');
        }
        ...
        for (final poi in allPois) {
          ...
          processedPois++;
          if (mounted && allPois.isNotEmpty) {
            final progress = 0.1 + ((processedPois / allPois.length) * 0.9);
            setState(() {
              _downloadProgress[packageId] = progress;
              _downloadStatusText[packageId] = 'POI: $processedPois / ${allPois.length}';
            });
          }
        }

        await OfflineCacheService.instance.replacePois(allPois);

        try {
          await _offlineService.markDownloaded(
            resourceType: 'poi',
            resourceId: allPois.first.id,
            sizeBytes: packageSizeMb * 1024 * 1024,
          );
        } catch (e) {
        ...
        setState(() {
          _downloadProgress[packageId] = 0.05;
          _downloadStatusText[packageId] = 'Telechargement de l\'annuaire local...';
        });

        final allServices = await _localServiceService.getAllServices(pageSize: 100);
        if (allServices.isEmpty) {
          throw Exception('Aucun service local recu depuis le backend.');
        }
        ...
        for (final service in allServices) {
          ...
          processedServices++;
          if (mounted && allServices.isNotEmpty) {
            final progress = 0.1 + ((processedServices / allServices.length) * 0.9);
            setState(() {
              _downloadProgress[packageId] = progress;
              _downloadStatusText[packageId] = 'Services: $processedServices / ${allServices.length}';
            });
          }
        }

        await OfflineCacheService.instance.replaceLocalServices(allServices);

        try {
          await _offlineService.markDownloaded(
            resourceType: 'service',
            resourceId: allServices.first.id,
            sizeBytes: packageSizeMb * 1024 * 1024,
          );
        } catch (e) {
      ...
      child: Row(
        children: [
          ...
          Switch(
            ...
            activeThumbColor: const Color(0xFF2EA043),
          ),
          const SizedBox(width: 8),

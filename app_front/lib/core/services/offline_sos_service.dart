...
class OfflineSosService {
  ...
  static Future<void> saveOfflineAlert(
    ...
    debugPrint('🚨 SOS Alert queued offline. Total queued: ${queue.length}');
  }
  ...
  static Future<void> syncOfflineAlerts(SosService sosService) async {
    ...
    debugPrint('📡 Attempting to sync ${queue.length} offline SOS alerts...');
    ...
    for (var alert in queue) {
      try {
        ...
        debugPrint('✅ Offline SOS Alert synced successfully.');
      } catch (e) {
        debugPrint('❌ Failed to sync Offline SOS Alert: $e');
